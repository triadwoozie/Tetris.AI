...
class Tetris:
    ...
    COLORS = {
        ...
        2: (60, 179, 113),  # Medium Sea Green (Green)
        3: (100, 149, 237), # Cornflower Blue (Blue)
        4: (255, 215, 0),   # Gold (Yellow)
        5: (255, 165, 0),   # Orange
        6: (147, 112, 219), # Medium Purple (Purple)
        7: (0, 206, 209)    # Dark Turquoise (Cyan)
    }
    ...
    def render(self):
        ...
        for y in range(Tetris.BOARD_HEIGHT):
            for x in range(Tetris.BOARD_WIDTH):
                color = self.COLORS[self.board[y][x]] if self.board[y][x] != Tetris.MAP_EMPTY else (0, 0, 0)
                pygame.draw.rect(self.screen, color, (x * 30, y * 30, 30, 30))
                pygame.draw.rect(self.screen, (255, 255, 255), (x * 30, y * 30, 30, 30), 1)  # Draw grid lines
...
if __name__ == "__main__":
    ...
    while not tetris_game.game_over:
        tetris_game.render()
        # Game loop code (input handling, updating state, etc.) goes here
